...
function basic_simplify_expr(expr) {
  ...
  if (is_number(part1) && is_number(part2)) {
    ...
    return make_expr(expr.type, basic_simplify_expr(part1), basic_simplify_expr(part2))
  }
}

function derive_sum(expr, variable) {
  return make_sum(derive(expr.operand1, variable), derive(expr.operand2, variable))
}
...
function derive_power(expr, variable) {
  ...
  const part2 = make_product(part1, make_power(f, make_sum(g, make_number(-1))))
  return part2
}
...
function derive_exponential(expr, variable) {
  ...
  const g = expr.operand2
  const gp = derive(g, variable)
  const part1 = make_product(make_number(special_functions("ln", parseFloat(display_expr(f)))), make_exponential(f, g))
